...
export class AbilitySystem implements ISystem {
  ...
  private breatheCycleTime: number = 0;
  private breatheCycleDuration: number = 4; // 4 second cycle (2s inhale, 2s exhale)
  private isInhaling: boolean = true;
  ...
  constructor() {
    this.abilities = {
      breathe: {
        name: 'Breathe',
        description: 'Constant damage aura that deals damage to stressors in range',
  ...
  getAffirmAmplification(): number {
    if (!this.affirmActive) return 1.0;
  ...
  update(deltaTime: number, context: SystemContext): void {
    ...
    this.updateLegacy(deltaTime, serenity, stressors, center);
  }
  ...
  updateLegacy(deltaTime: number, serenity: number, stressors: Stressor[], center: Vector2): void {
    ...
    // Align - Rhythmic modulator (always active)
    ...
    const cycleProgress = (this.breatheCycleTime % this.breatheCycleDuration) / this.breatheCycleDuration;
    
    // Inhale for first half, exhale for second half
    this.lastInhalingState = this.isInhaling;
    this.isInhaling = cycleProgress < 0.5;
    
    // Track breathing state for visual/effect purposes
    if (this.isInhaling) {
      // During inhale, build up
      this.breatheHeld = true;
      const inhaleProgress = cycleProgress * 2; // 0 to 1 during inhale
  ...
  justTransitionedToExhale(): boolean {
    return this.lastInhalingState && !this.isInhaling;
  }
  ...
  getBreatheCycleProgress(): number {
    ...
    const cycleProgress = (this.breatheCycleTime % this.breatheCycleDuration) / this.breatheCycleDuration;
    if (this.isInhaling) {
      return cycleProgress * 2; // 0 to 1 during inhale
    } else {
      return 1 - ((cycleProgress - 0.5) * 2); // 1 to 0 during exhale
    }
  }
  ...
  getBreatheRawCycleProgress(): number {
    ...
    return (this.breatheCycleTime % this.breatheCycleDuration) / this.breatheCycleDuration;
  }
